...
-------------------------------------------------------------------------*/
class Ves_Contentslider_Helper_Data extends Mage_Core_Helper_Abstract {
	...
	function get( $attributes = NULL)
	{
		...
		{	    	
			$tags = array('ves_contentslider', 'effect_setting', 'file_source_setting','image_source_setting', 'main_contentslider');
			foreach($tags as $tag){
				if(Mage::getStoreConfig("ves_contentslider/$tag/$var")!=""){
					$data[$var] =  Mage::getStoreConfig("ves_contentslider/$tag/$var");
				}
			}
        ...
        return Mage::getSingleton('ves_contentslider/config')->getBaseMediaUrl() . $url;
    }
	...
	public function getCategoriesList( $default=true ){
		...
		$collection =  Mage::getModel('ves_contentslider/category')->getCollection();
	...
	public function getTagUrl($tag){
		return Mage::getBaseUrl().Mage::getStoreConfig('ves_contentslider/general_setting/route')."/tag/".$tag.".html";
	}
	
	public function getPostURL( $id ){
		return Mage::getBaseUrl().Mage::getModel('core/url_rewrite')->loadByIdPath('vescontentslider/post/'.$id)->getRequestPath();
	}
	
	public function getBrandById( $id ){
		return Mage::getModel("ves_contentslider/brand")->load( $id ) ;
	}
